...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        // Remove o prefixo "Bearer " pra pegar só o token
        String token = authHeader.substring(7);
        String email = jwtService.extrairEmail(token);

        // Se conseguiu extrair um email E o usuário ainda não está autenticado nesse contexto
        if (email != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            if (jwtService.tokenValido(token, userDetails.getUsername())) {
                ...
                // "Autentica" o usuário pro resto dessa requisição
